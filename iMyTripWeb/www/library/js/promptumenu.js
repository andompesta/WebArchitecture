...
(function ($) {
    $.fn.promptumenu = function (options) {
        ...
        return this.each(function () {
            ...
            if ($this.data('promptumenu') != undefined) {
                ...
                if (settings.resizer != null)
                {
                    settings.resizer.resize(function (elem) {



                    });
                }
                if (settings.fixeddim == false) {
                    cells.width = properties.width / settings.columns;
                    cells.height = (properties.height - 75) / settings.rows;
                } else {
                    cells.width = settings.cellwidth;
                    cells.height = settings.cellheight;
                }
                ...
                if (settings.title != undefined) {
                    $this.parent('.promptumenu_window').append('<div style="text-align: center; border-top-left-radius: 9px; border-top-right-radius: 9px; color: '+settings.titlecolor+'; background-color:'+settings.titlebackcolor+';   ">'+settings.title+'</div>')
                ...
                $this.children('li').each(function () {
                    ...
                    if (settings.direction == 'vertical') {
                        ...
                        $li.css({
                            'top'  : Math.round((cursor.y * cells.height - cells.height / 2) - ($li.height() / 2) + (cursor.page - 1) * properties.height) - settings.topoffset,
                            'left' : Math.round((cursor.x * cells.width - cells.width / 2) - ($li.width() / 2)) - settings.leftoffset
                        });
                        ...
                        $li.find('div').bind('load', function () {
                        ...
                        $li.css({
                            'top'  : Math.round((cursor.y * cells.height - cells.height / 2) - ($li.height() / 2)) - settings.topoffset,
                            'left' : Math.round((cursor.x * cells.width - cells.width / 2) - ($li.width() / 2) + (cursor.page - 1) * properties.width) - settings.leftoffset
                        });
                ...
                if (cells.pages > 1 && settings.pages == true) {
                    ...
                    for (i = 2; i <= cells.pages; i++) {
                        page_links = page_links + '<a id="PAGENAV">Page ' + i + '</a>';
                    }
                ...
                $this.bind('mousedown.promptumenu', function (mdown) {
                    ...
                    $(document).bind('mouseup.promptumenu', function (mup) {
                        ...
                        var yy;
                        try {
                            xtime = (delta_end.time - delta_start.time)
                        } catch (e) {
                            return
                        }
                        ;
                        try {
                            xx = (delta_end.x - delta_start.x)
                        } catch (e) {
                            return
                        }
                        ;
                        try {
                            yy = (delta_end.y - delta_start.y)
                        } catch (e) {
                            return
                        }
                        ;
